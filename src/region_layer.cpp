...
region_layer* make_region_layer(int batch, int w, int h, int n, int classes, int coords){
  region_layer* l = new region_layer();
...
void region_layer::resize(int w, int h){
  region_layer* l = this;
...
void delta_region_class(float *output, float *delta, int index, int cl, int classes,
                        tree *hier, float scale, int stride, float *avg_cat, int tag){
    int i, n;
    if(hier){
        float pred = 1;
        while(cl >= 0){
            pred *= output[index + stride*cl];
            int g = hier->group[cl];
            int offset = hier->group_offset[g];
            for(i = 0; i < hier->group_size[g]; ++i){
                delta[index + stride*(offset + i)] = scale * (0 - output[index + stride*(offset + i)]);
            }
            delta[index + stride*cl] = scale * (1 - output[index + stride*cl]);
        ...
        *avg_cat += pred;
        ...
        for(n = 0; n < classes; ++n){
            delta[index + stride*n] = scale * (((n == cl)?1 : 0) - output[index + stride*n]);
            if(n == cl) *avg_cat += output[index + stride*n];
        }
    }
}
...
void region_layer::forward(network net){
  region_layer* l = this;
    ...
    for (b = 0; b < l->batch; ++b) {
        if(l->softmax_tree){
            ...
            for(t = 0; t < 30; ++t){
                ...
                if(!truth.x) break;
                int cl = net.truth[t*(l->coords + 1) + b*l->truths + l->coords];
                float maxp = 0;
                int maxi = 0;
                if(truth.x > 100000 && truth.y > 100000){
                    for(n = 0; n < l->n*l->w*l->h; ++n){
                        ...
                        float p = scale*get_hierarchy_probability(l->output + class_index, l->softmax_tree, cl, l->w*l->h);
                        if(p > maxp){
                            maxp = p;
                            maxi = n;
                        }
                    ...
                    delta_region_class(l->output, l->delta, class_index, cl, l->classes, l->softmax_tree, l->class_scale, l->w*l->h, &avg_cat, !l->softmax);
        ...
        for (j = 0; j < l->h; ++j) {
            for (i = 0; i < l->w; ++i) {
                for (n = 0; n < l->n; ++n) {
                    ...
                    float best_iou = 0;
                    for(t = 0; t < 30; ++t){
                      //truth = float_to_box(net.truth + t*(l->coords + 1) + b*l->truths, 1);
                      //if(!truth.x) break;
                      float iou;// = box_iou(pred, truth);
                      if (iou > best_iou) {
                        best_iou = iou;
...
void region_layer::backward(network net){
    /*
       int b;
